...
async function deleteListing(id){
  ...
  const urls=[p.cover_image_url,...(Array.isArray(p.image_urls)?p.image_urls:[])].filter(Boolean);
  ...
  alert('Listing deleted successfully.'); window.setPath('/profile');
}
...
function injectListingActions(){
  ...
  currentUser().then(async user=>{
    ...
    if(!root.querySelector('.s2-contact')){ const btn=document.createElement('button'); btn.className='s2-contact'; btn.textContent='Contact seller'; btn.setAttribute('data-s2-contact',id); root.appendChild(btn); }
...
async function openContact(id){
  const user=await currentUser(); if(!user){ window.setPath('/auth'); return; }
  const data=await sellerContact(id).catch(()=>null); if(!data){alert('Seller contact details could not be loaded.');return;}
  ...
  modal.addEventListener('click',async e=>{
    ...
    const reveal=e.target.closest('[data-s2-reveal]'); if(reveal){const type=reveal.dataset.s2Reveal,value=type==='phone'?data.phone:data.email,out=modal.querySelector('#s2-'+type+'-value');out.textContent=value||('Seller has not added a '+type+' yet.');if(value)await recordView(id,data.seller_id,type);}
...
function injectMessagesSummary(){
  if(route()!=='/messages'||document.querySelector('.s2-contact-summary')) return;
  currentUser().then(async user=>{ if(!user)return; const r=await db.from('contact_views').select('contact_type').eq('buyer_id',user.id); if(r.error)return; const phone=(r.data||[]).filter(x=>x.contact_type==='phone').length,email=(r.data||[]).filter(x=>x.contact_type==='email').length,root=document.querySelector('.content')||document.querySelector('.screen'); if(!root)return; const box=document.createElement('section');box.className='s2-contact-summary';box.innerHTML='<b>Seller contact views</b><br><span>'+phone+' phone number'+(phone===1?'':'s')+' · '+email+' email'+(email===1?'':'s')+' viewed by you</span>';root.insertBefore(box,root.firstChild); }).catch(()=>{});
}

/* The profile page intentionally has no promotional phone/email reminder box. */
function sync(){ const r=route(); if(r==='/messages')injectMessagesSummary(); if(r.startsWith('/listing/'))injectListingActions(); document.querySelectorAll('.heart').forEach(x=>{x.style.color='#e53935';x.style.setProperty('color','#e53935','important')}); }
...
const observer=new MutationObserver(()=>{clearTimeout(observer._t);observer._t=setTimeout(sync,60)}); observer.observe(document.body,{childList:true,subtree:true});
