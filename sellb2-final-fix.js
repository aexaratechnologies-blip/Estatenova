(function(){
  ...
  function install(){
    ...
    window.setPath=go;
    window.__sellb2Navigate=go;

    // Keep navigation in the SPA. location.assign() was causing the splash screen
    // to restart every time a bottom-nav item was pressed.
    document.addEventListener('click',function(ev){
      ...
      if(tab){const p=pathFromText(tab.textContent||'');if((tab.textContent||'').trim().toLowerCase()==='everything'){ev.preventDefault();ev.stopImmediatePropagation();window.st.cat='all';window.st.type='all';go('/properties');return}if(p){ev.preventDefault();ev.stopImmediatePropagation();window.st.cat=p==='/vehicles'?'vehicle':p==='/businesses'?'business':'property';window.st.type='all';go(p);return}}
      const brand=el.closest('.appbar .brand');
      if(brand){ev.preventDefault();ev.stopImmediatePropagation();go('/');return}
      const sign=el.closest('.appbar .signin');
      if(sign){ev.preventDefault();ev.stopImmediatePropagation();go(window.st.user?'/profile':'/auth');return}
      const detail=el.closest('.detailactions .btn');
      if(detail&&/^\/listing\//.test(location.pathname)){
        const id=decodeURIComponent(location.pathname.split('/')[2]);
        const t=(detail.textContent||'').toLowerCase();
    ...
    window.chatStart=async function(id){
      ...
      try{
        ...
        go('/messages/'+encodeURIComponent(cid));
      }catch(e){toast(e?.message||'Could not contact seller.')}
    };

    // Core chat UI already has the correct messages-table shape. Use the direct insert
    // path here so sending does not depend on a missing/incorrect RPC wrapper.
    window.sendMsg=async function(e,id){
      ...
      if(!body)return;
      try{
        const r=await window.db.from('messages').insert({conversation_id:id,sender_id:window.st.user.id,body});
        if(r.error)throw r.error;
    ...
    window.addEventListener('popstate',function(){
      ...
      Promise.resolve(window.render()).then(()=>{if(/^\/(properties|vehicles|businesses)$/.test(location.pathname))window.load()});
    });
  }
  wait();
})();
