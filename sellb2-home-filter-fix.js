...
(function(){
  ...
  function install(){
    ...
    window.__sellb2RunSearch=function(){
      window.st.cat='all';window.st.type='all';navigate('/properties');window.load();
    };
    ...
    window.applyFilters=async function(){
      ...
      s.type=s.type||'all';navigate(targetPath());await window.load();
    };
    ...
    /* One normal click listener is enough. The old pointerdown/touchstart capture
       path could cancel the real mobile click and leave the SPA half-rendered. */
    document.addEventListener('click',function(ev){
      ...
      if(filterButton){ev.preventDefault();ev.stopPropagation();window.__sellb2OpenFilters(ev);return;}
      const searchButton=el.closest('.homehero .heroactions .btn.primary');
      if(searchButton){ev.preventDefault();ev.stopPropagation();window.__sellb2RunSearch();return;}
    },false);
    ...
    document.addEventListener('keydown',function(ev){
      ...
      ev.preventDefault();window.st.locality=input.value||'';
      if(input.closest('.homehero'))window.__sellb2RunSearch();else window.load();
    },false);
